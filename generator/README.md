...
- 생성 약 5분, 검증 약 1분(노트북 1대 기준, 80,000명 규모).
...
1. **카탈로그**: 공간 60곳(지역·장르·규모), 행사(주당 수는 시나리오 구간별, 시작은 목~토 밤에 몰림, 공개는 시작 10~28일 전). 가격대는 무료 / 15,000~25,000원 / 40,000~60,000원. 관측 마지막 날에도 신청 가능한 행사가 최소 1건 있도록 보장한다(요일 가중만 따르면 마지막 날이 일요일 등 저빈도 요일일 때 그날 시작하는 행사가 0건이 될 수 있어 신청·결제가 끊긴다).
...
5. **세션**: 방문일마다 1~3세션(단골층 1~4), 시작 시각은 요일별 시 분포. 자동 로드 세션(체류 0, 화면 1개, 2초 이내)을 seed 비중만큼 섞는다. 세션 시작 시각은 관측 마지막 날 23시대까지 그대로 두고, 세션이 관측 기간 끝을 넘기면 시작 시각이 아니라 길이(체류)만 줄여 날짜 밖으로 새지 않게 한다(시작 시각을 당기면 마지막 날 늦은 시간대 데이터가 통째로 비게 된다).
...
| 기기 / 사람 | 82,590 / 80,000 |
| 회원 | 7,717 (9.6%) |
| 신청 / 결제 / 취소 | 40,945 / 19,296 / 3,885 |
| 리텐션 W1 / W2 / W4 / W8 / W12 | 0.219 / 0.140 / 0.101 / 0.086 / 0.078 |
| 방문 → 가입 / 회원 2기기 | 0.096 / 0.336 |
| 상세 → 신청 / 신청 → 결제 / 취소율 | 0.057 / 0.664 / 0.095 |
| 광고 세션 비중(집행일) / 자동 로드 | 0.202 / 0.072 |
| 캠페인별 광고 세션 ÷ 클릭 | 0.56 ~ 0.87 |
| 일 방문 사람 p10 / p50 / p90 | 322 / 1,945 / 3,608 |

단언 30개 통과(경계 점검 4건 포함), 같은 seed 재실행 시 SHA256 동일(`data/MANIFEST.txt`). 생성 304초 · 최대 RSS 258MB, 검증 73초 · 최대 RSS 1.27GB(노트북 1대, `/usr/bin/time -l` 측정).
