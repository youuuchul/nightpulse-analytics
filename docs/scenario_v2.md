...
런칭 1년차 서울 나이트라이프 플랫폼 Placewave. 1.0(탐색 → 행사 신청 → 결제)에 **공간 등록/파트너 계약**, **소비자 구독**, **B2B 계약 매출** 축을 더한다. 모든 수치는 가상 시나리오와 분포 규칙으로 생성한다. 실제 서울 상권 이름·좌표는 공개 지리 정보이며, 상호·브랜드는 전부 가상이다.
