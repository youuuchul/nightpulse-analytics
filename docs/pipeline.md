...
전체 실행 1회에 쿼리 처리량 약 7.3GB(52주·이벤트 약 827만 행 기준, 정제 2.2GB·세션 1.4GB가 대부분). 표 41개(마트 18개) 전체 재생성 기준. 단계당 상한 `NP_MAX_BYTES` 기본 5GB. 적재는 무료다. 무료 한도(월 1TB 쿼리·10GB 저장) 안에서 하루 여러 번 돌려도 된다.
