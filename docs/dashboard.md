...
| 행사·결제 | 공간별 | 지역·장르 | 조회된 공간·상세 조회·신청·결제 | 공간 상세 조회 추이 | 공간 순위 | daily_venue |
...
`funnel_daily` 의 단계는 누적 조건이다(랜딩 ⊇ 상세 ⊇ 로그인·가입 ⊇ 신청 화면 ⊇ 결제). 아래 Σ 는 필터(기간·세그먼트 3축) 적용 후 단계별 persons 합이다.

| 지표 | 산식 | 단위 |
|---|---|---|
| 전체 전환율 | Σ payment ÷ Σ landing | 사람·일 |
| 최대 이탈 단계 | 랜딩 대비 도달률 r(k) = Σ step_k ÷ Σ landing 에서 r(k−1) − r(k) 가 가장 큰 k. 값은 그 차(%p) | 사람·일 |
| 가장 크게 변한 단계 | 직전 단계 대비 전환율 c(k) = Σ step_k ÷ Σ step_(k−1) 의 직전 동일 길이 기간 대비 차가 절댓값 최대인 k. 직전 기간이 데이터 범위 밖이면 비운다 | %p |
| 단계 전환율 추이 | 기간 28일 이상 주(월요일) 버킷, 미만 일 버킷마다 c(apply_view), c(payment). 분모 0인 버킷은 선을 끊는다 | 사람·일 |
| 세그먼트별 퍼널 | 행 = 전체 + 세그먼트 값(선택한 축의 다른 값은 숨김), 칸 = r(k). 색은 열 안 최소~최대 명도 | 사람·일 |
...
`?page=about`. 탭이 아니라 헤더의 '소개'/'대시보드' 버튼으로 오가는 별도 화면이며, 오갈 때 대시보드 URL 상태(탭·기간·필터)를 유지하고 브라우저 뒤로 가기가 동작한다. 대시보드 안에 설명 문장을 넣지 않기 위한 자리다.
...
`npm run build && npm run capture -- --all` 이 `dashboard/captures/` 에 탭별 라이트 화면과 보기 전환·1일·1년·세그먼트 조합·드릴다운 카드·데이터 없음·다크·폰 폭(390px)·소개 페이지를 저장한다. README 대표 이미지는 `docs/img/` 에 최적화해 복사한다.
